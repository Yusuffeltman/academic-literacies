// src/components/contact-notebook.js
// ─────────────────────────────────────────────
// Contact Session Notebook — student-facing
...
const MAX_UPLOAD_BYTES = 10 * 1024 * 1024; // 10 MB limit for contact notebook uploads

const CONTACT_SESSIONS = Object.values(SESSIONS)
  .filter((s) => s && s.type === 'contact')
  .sort((a, b) => _unitNum(a.unit) - _unitNum(b.unit));

const CONTACT_BY_UNIT = new Map(CONTACT_SESSIONS.map((s) => [s.unit, s]));
const CONTACT_BY_ID = new Map(CONTACT_SESSIONS.map((s) => [s.id, s]));
...
export function renderContactNotebook(opts = {}) {
...
function _resolveActiveUnit(opts) {
  const fromSession = opts?.sessionId && CONTACT_BY_ID.get(opts.sessionId)?.unit;
  const fromUnitOpt = opts?.unitId;
  const fromState = STATE.contactNotebook?.lastUnitId;
  ...
  if (candidate && CONTACT_BY_UNIT.has(candidate)) return candidate;

  return CONTACT_SESSIONS[0]?.unit || 'u1';
}

function _renderPage(area) {
  const session = CONTACT_BY_UNIT.get(_activeUnitId) || CONTACT_SESSIONS[0];
  if (!session) {
    area.innerHTML = '<div style="padding:40px;color:var(--muted);">No contact sessions available.</div>';
    return;
  }
  ...
  const analytics = STATE.contactNotebook?.analytics || {};
  const archivedUnits = Object.entries(STATE.contactNotebook?.archivedUnits || {})
    ...
    <div class="contact-notebook anim-fade">
      <section class="cn-workspace-head">
        <div class="cn-workspace-main">
          <div class="cn-hero-label">${_esc(session.phase || 'Contact Session')} · ${_esc(unit?.badge || `Unit ${_unitNum(session.unit)}`)}</div>
      ...
      <nav class="cn-tabs" id="cn-tabs">
        ${CONTACT_SESSIONS.map((s) => _tabHTML(s, s.unit === _activeUnitId)).join('')}
      </nav>
...
function _bindEvents(area) {
  const session = CONTACT_BY_UNIT.get(_activeUnitId);
  if (!session) return;
  ...
  area.querySelectorAll('[data-cn-unit]').forEach((btn) => {
    btn.addEventListener('click', () => {
      ...
      _ensureState();
      STATE.contactNotebook.lastUnitId = unitId;
      STATE.contactNotebook.lastSessionId = CONTACT_BY_UNIT.get(unitId)?.id || null;
      saveState();
      _renderPage(area);
      _bindEvents(area);
    });
  ...
  if (!window._contactNotebookUploadSyncBound) {
    window._contactNotebookUploadSyncBound = true;
    window.addEventListener('notebook-upload-complete', (event) => {
      const detail = event?.detail || {};
      if (detail.notebookType !== 'contact') return;
      const activeSession = CONTACT_BY_UNIT.get(_activeUnitId);
...
function _handleFieldChange(session, field, value) {
  ...
  STATE.contactNotebook.lastUnitId = session.unit;
  STATE.contactNotebook.lastSessionId = session.id;
  _refreshNotebookAnalytics();
  persistLocalStateSoon('contact-notebook-input');
...
async function _handleUpload(sessionId) {
  ...
  try {
    ...
    await writeUploadSuccessEvent({
      user: STATE.user || {},
      profile: _analyticsProfile(),
      scope: 'contact-notebook',
      unitId: session.unit,
      sessionId: session.id,
      asset,
      source: 'contact-notebook-upload',
    }).catch(() => {});
...
function _renderSearchResults() {
  ...
  const results = CONTACT_SESSIONS.map((s) => {
...
function _downloadCurrentSession(sessionId) {
  ...
  a.href = url;
  a.download = `contact-notebook-${session.id}-${new Date().toISOString().slice(0, 10)}.txt`;
  a.click();
...
function _ensureState() {
  if (!STATE.contactNotebook || typeof STATE.contactNotebook !== 'object') {
    STATE.contactNotebook = { entries: {}, lastUnitId: null, lastSessionId: null, archivedUnits: {}, analytics: {} };
  }
  if (!STATE.contactNotebook.entries || typeof STATE.contactNotebook.entries !== 'object') {
    STATE.contactNotebook.entries = {};
  }
  if (!STATE.contactNotebook.archivedUnits || typeof STATE.contactNotebook.archivedUnits !== 'object') {
    STATE.contactNotebook.archivedUnits = {};
  }
  if (!STATE.contactNotebook.analytics || typeof STATE.contactNotebook.analytics !== 'object') {
    STATE.contactNotebook.analytics = {};
  }
}

function _ensureEntry(session) {
  const entries = STATE.contactNotebook.entries;
...
function _getEntry(sessionId) {
  return STATE.contactNotebook?.entries?.[sessionId] || null;
}

function _isUnitArchived(unitId) {
  return Boolean(STATE.contactNotebook?.archivedUnits?.[unitId]);
}

function _archiveMeta(unitId) {
  return STATE.contactNotebook?.archivedUnits?.[unitId] || null;
}

function _refreshNotebookAnalytics() {
  _ensureState();
  const entries = Object.values(STATE.contactNotebook.entries || {});
  ...
  STATE.contactNotebook.analytics = {
    ...
    archivedUnits: Object.keys(STATE.contactNotebook.archivedUnits || {}).length,
    lastUpdatedAt: latestUpdatedAt,
  };

  if (!STATE.progress) STATE.progress = {};
  STATE.progress.contactNotebookAnalytics = { ...STATE.contactNotebook.analytics };
}
...
function _archiveUnit(unitId) {
  _ensureState();
  const sessions = CONTACT_SESSIONS.filter((session) => session.unit === unitId);
  const entries = sessions.map((session) => _getEntry(session.id)).filter(Boolean);
  STATE.contactNotebook.archivedUnits[unitId] = {
...
function _unarchiveUnit(unitId) {
  if (!STATE.contactNotebook?.archivedUnits?.[unitId]) return;
  delete STATE.contactNotebook.archivedUnits[unitId];
  _refreshNotebookAnalytics();
}
